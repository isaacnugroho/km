...
| `get_system_status`       | *None*                                                                                               | `{ "active_branch": string, "learning_ontologies": [Binding], "pending_exceptions_count": int, "pending_mrs_count": int, "branch_merge_policy": string, "pending_branch_merges_count": int }` | Includes effective `branch_merge.policy` (`auto_merge_exception` default). `pending_mrs_count` from **source** governance graphs. `pending_branch_merges_count` from `.km/pending-merge-*.json`. |
| `propose_branch_merge`    | `source_branch`, optional `target_branch`, optional `event_fingerprint`                              | `{ "event_id", "status", "approval_command", ... }`                                                                                                                                           | §5.3 Case graph sync after Git merge; use while MCP is running (not `km merge-resolve`).                                                                                                         |
| `resolve_branch_merge`    | `event_id`, `resolution` (`MERGE` \                                                                  | `KEEP_ISOLATED` \                                                                                                                                                                             | `DELETE`)                                                                                                                                                                                        |
...
- **`km://case/pending-merges`**: Lists pending §5.3 branch merge prompts; `km://case/pending-merges/{event-id}` for one prompt.
