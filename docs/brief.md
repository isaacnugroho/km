...
| `status`                     | *None*                                                                                               | `{ "active_branch", "learning_ontologies", "pending_exceptions_count", "pending_mrs_count", "branch_merge_policy", "pending_branch_merges_count", "pending_branch_merges": [...] }` | Same JSON as `km status`. `pending_branch_merges` includes `approval_command` per pending prompt.                                                                                 |
| `export_case`                | *None*                                                                                               | `{ "status": "success", "export_path": string }`                                                                                                                                    | Same as `km export-case`.                                                                                                                                                         |
| `sync_pending_branch_merges` | `source_branch`, optional `target_branch`, optional `event_fingerprint`                              | `{ "event_id", "status": "PENDING_RESOLUTION" \                                                                                                                                     | "ALREADY_SYNCED" \                                                                                                                                                                |
